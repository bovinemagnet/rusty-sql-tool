...
pub struct ColumnDefinition {
    ...
    /// Where the column's values come from when PostgreSQL supplies them rather than the writer.
    pub generation: ColumnGeneration,
}

/// FR2-001 asks for the column's default. An identity column and a stored generated column both
/// produce values without one, and reporting either as a plain `DEFAULT` would misstate what the
/// column is: identity draws from a sequence, and a generated column cannot be written to at all.
...
pub enum ColumnGeneration {
    ...
    IdentityByDefault,
    /// `GENERATED ALWAYS AS (…) STORED` — `attgenerated = 's'`. The expression arrives in
    /// `default`, because `pg_attrdef` is where PostgreSQL keeps it.
    Stored,
}
...
impl ObjectDefinition {
    ...
    pub fn sections(&self, object: &DatabaseObject) -> Vec<DefinitionSection> {
        match self {
            ...
            Self::View(view) => {
                let mut sections = Vec::new();
                // Otherwise a materialised view is indistinguishable from a view, though the two
                // are different objects: one stores rows, the other stores only the query (§5).
                if view.materialised {
                    sections.push(DefinitionSection::Note {
                        text: "Materialised view".into(),
                    });
                }
...
///
/// Widths count characters rather than bytes, matching `editor_lines` in the UI: the surface is
/// monospace, so an accented identifier occupies one cell however many bytes it takes.
///
/// "Last field" is judged per row rather than against the widest row, which is safe only because
/// the line is right-trimmed: a short row's final field is padded like any other, and the padding
/// is then removed. Drop the `trim_end` and short rows would gain trailing spaces.
...
fn column_lines(columns: &[ColumnDefinition]) -> Vec<String> {
    let rows: Vec<Vec<String>> = columns
        ...
        .map(|column| {
            ...
            if let Some(clause) = generation_clause(column) {
                if !suffix.is_empty() {
                    suffix.push(' ');
                }
                suffix.push_str(&clause);
            }
...
/// How the column gets its value, in PostgreSQL's own words. An identity column has no
/// `pg_attrdef` entry at all, and a stored generated column has one that is not a default, so
/// neither may be printed as `DEFAULT …` (FR2-001).
fn generation_clause(column: &ColumnDefinition) -> Option<String> {
    match column.generation {
        ColumnGeneration::IdentityAlways => Some("GENERATED ALWAYS AS IDENTITY".to_owned()),
        ColumnGeneration::IdentityByDefault => Some("GENERATED BY DEFAULT AS IDENTITY".to_owned()),
        ColumnGeneration::Stored => Some(format!(
            "GENERATED ALWAYS AS {} STORED",
            column.default.as_deref().unwrap_or("(?)")
        )),
        ColumnGeneration::None => column
            .default
            .as_ref()
...
mod tests {
    ...
    /// FR2-001. An identity column has no `pg_attrdef` entry and a stored generated column's entry
    /// is not a default, so neither may be reported as `DEFAULT …`.
    #[test]
    fn identity_and_generated_columns_are_not_reported_as_defaults() {
        let definition = ObjectDefinition::Table(TableDefinition {
            columns: vec![
                ColumnDefinition {
                    generation: ColumnGeneration::IdentityAlways,
                    ..column(1, "id", "bigint", false, None)
                },
                ColumnDefinition {
                    generation: ColumnGeneration::IdentityByDefault,
                    ..column(2, "code", "integer", false, None)
                },
                ColumnDefinition {
                    generation: ColumnGeneration::Stored,
                    ..column(3, "total", "numeric", true, Some("(price * quantity)"))
                },
                column(4, "active", "boolean", false, Some("true")),
            ],
            ..TableDefinition::default()
        ...
        let DefinitionSection::Rows { lines, .. } =
            &definition.sections(&object("invoice", ObjectKind::Table))[0]
        else {
        ...
        assert_eq!(
            lines,
            &vec![
                "id      bigint   NOT NULL GENERATED ALWAYS AS IDENTITY".to_owned(),
                "code    integer  NOT NULL GENERATED BY DEFAULT AS IDENTITY".to_owned(),
                "total   numeric  GENERATED ALWAYS AS (price * quantity) STORED".to_owned(),
                "active  boolean  NOT NULL DEFAULT true".to_owned(),
            ]
        );
    }

    /// The surface is monospace, so alignment counts characters. Measuring bytes would push every
    /// following field right by the extra bytes an accented identifier costs.
    #[test]
    fn alignment_counts_characters_rather_than_bytes() {
        let definition = ObjectDefinition::Table(TableDefinition {
            columns: vec![
                column(1, "café", "text", true, None),
                column(2, "id", "bigint", false, None),
            ],
            ..TableDefinition::default()
        });

        let DefinitionSection::Rows { lines, .. } =
            &definition.sections(&object("menu", ObjectKind::Table))[0]
        else {
            panic!("expected a rows section");
        };

        assert_eq!(lines[0], "café  text");
        assert_eq!(lines[1], "id    bigint  NOT NULL");
    }

    /// §5: a materialised view stores rows where a view stores only a query, so the two must not
    /// render identically.
    #[test]
    fn a_materialised_view_says_so() {
        ...
        let sections =
            definition.sections(&object("customer_totals", ObjectKind::MaterialisedView));
